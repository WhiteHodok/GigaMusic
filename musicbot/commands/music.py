...
class Music(commands.Cog):
    ...
    async def _stop(self, ctx: Context):
        ...
        await ctx.send("Остановлены все сессии :octagonal_sign:")
    ...
    async def _skip(self, ctx: Context):
        ...
        await ctx.send("Скипнута песня :fast_forward:")
    ...
    async def _prev(self, ctx: Context):
        ...
        if audiocontroller.prev_song():
            await ctx.send("Играть предыдующую песню :track_previous:")
        else:
            await ctx.send("Нема предыдущей песни.")
    ...
    async def _songinfo(self, ctx: Context):
        ...
        if song is None:
            await ctx.send("Нет песен играющих сейчас.")
            return
    ...
    async def _volume(
        ...
        if value is None:
            await ctx.send(
                "Громкость бота: {}% :speaker:".format(audiocontroller.volume)
            )
            return

        if value > 100 or value < 0:
            await ctx.send("Error: Громкость должна быть числом в отрезке 1-100")
            return

        if audiocontroller.volume >= value:
            await ctx.send("Громкость установлена на {}% :sound:".format(str(value)))
        else:
            await ctx.send("Громкость установлена на {}% :loud_sound:".format(str(value)))
